...
public class TreeImpl<E extends Comparable<E>> implements Tree<E> {
    ...
    public boolean add(E value) {
        ...
        int nodeLevel = nodeAndParent.parent == null ? 0 : nodeAndParent.parent.nodeLevel + 1;
        ...
        Node<E> node = new Node<>(value, nodeLevel);
...
//    }
//
    private boolean isBalancedTree(Node<E> node){
        if (node.isLeaf() || node.hasOnlyOneChild()) {
           return Math.abs(node.nodeLevel - maxLevel) < 2;
        }

        boolean isBalancedLeft = true;
        boolean isBalancedRight = true;

        isBalancedLeft = isBalancedLeft && isBalancedTree(node.getLeftChild());
        isBalancedRight = isBalancedRight && isBalancedTree(node.getRightChild());
